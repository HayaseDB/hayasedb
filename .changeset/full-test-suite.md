...
Add a full automated test suite across the monorepo: Vitest unit and integration projects, Playwright end-to-end coverage, and the CI jobs that run them
...
- CI: `ci.yml` splits into `ci`, `unit`, `integration` and `e2e` jobs on `ubuntu-24.04` with job-suffixed Turbo cache keys, read-only default permissions, an lcov coverage artifact and a Playwright report artifact; `--affected` filtering is dropped so every run covers the whole graph. Turbo gains cacheable `test` inputs plus uncached `test:integration` and `test:e2e` tasks with explicit `passThroughEnv`
